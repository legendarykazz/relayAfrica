...
export default function CampaignsManager({ apiKey, initialData }: { apiKey: string, initialData?: any }) {
  ...
  return (
    <div className="animate-fade">
      ...
      {showNew ? (
        <div className="panel glass" style={{ display: 'flex', gap: '2rem', flexWrap: 'wrap' }}>
           
           {/* Left Form */}
           <div style={{ flex: '1 1 300px', display: 'flex', flexDirection: 'column', gap: '1.2rem' }}>
             <h3 style={{ fontSize: '1.1rem', fontWeight: 600, borderBottom: '1px solid rgba(255,255,255,0.05)', paddingBottom: '0.5rem' }}>Campaign Settings</h3>
             ...
             <div>
               <label style={{ display: 'block', fontSize: '0.85rem', color: '#94a3b8', marginBottom: '0.5rem' }}>Target Audience</label>
               <select className="input-styled" value={formData.audienceListId} onChange={e => setFormData({...formData, audienceListId: e.target.value})} style={{ appearance: 'none', backgroundColor: '#0f172a' }}>
                 {lists.map(l => <option key={l.id} value={l.id}>{l.name}</option>)}
                 {lists.length === 0 && <option value="">No lists available (Create one in Contacts!)</option>}
               </select>
             </div>

             <div style={{ marginTop: '1rem' }}>
               <h3 style={{ fontSize: '1.1rem', fontWeight: 600, borderBottom: '1px solid rgba(255,255,255,0.05)', paddingBottom: '0.5rem', marginBottom: '1.2rem' }}>Email Design</h3>
               <label style={{ display: 'block', fontSize: '0.85rem', color: '#94a3b8', marginBottom: '0.5rem' }}>Subject Line</label>
               <input type="text" className="input-styled" placeholder="Exclusive early access inside!" value={formData.subject} onChange={e => setFormData({...formData, subject: e.target.value})} />
             </div>
             
             <div>
               <label style={{ display: 'block', fontSize: '0.85rem', color: '#94a3b8', marginBottom: '0.5rem' }}>HTML Content</label>
               <textarea className="input-styled" rows={12} placeholder="<h1>Hello!</h1>..." value={formData.html} onChange={e => setFormData({...formData, html: e.target.value})} style={{ fontFamily: 'monospace' }} />
             </div>

             <button className="btn btn-primary" onClick={handleLaunch} disabled={launching} style={{ display: 'flex', justifyContent: 'center', alignItems: 'center', gap: '0.5rem', padding: '1rem', marginTop: '1rem' }}>
               {launching ? <Loader2 className="animate-spin" size={20} /> : <Rocket size={20} />}
               {launching ? "Sending to Queue..." : "Dispatch Campaign"}
             </button>
           </div>

           {/* Right Preview */}
           <div style={{ flex: '1 1 300px', display: 'flex', flexDirection: 'column' }}>
             <h3 style={{ fontSize: '1.1rem', fontWeight: 600, borderBottom: '1px solid rgba(255,255,255,0.05)', paddingBottom: '0.5rem', marginBottom: '1.2rem' }}>Live Preview</h3>
             <div style={{ flexGrow: 1, background: '#ffffff', borderRadius: '0.5rem', padding: '2rem', color: '#000000', border: '1px solid rgba(255,255,255,0.1)', overflowY: 'auto', minHeight: '400px' }}>
               {formData.html ? (
              ...
              <tbody>
                {campaigns.map(c => (
                  <tr key={c.id} style={{ borderBottom: '1px solid rgba(255,255,255,0.05)' }}>
                    <td style={{ padding: '1rem 0.5rem', display: 'flex', alignItems: 'center', gap: '0.8rem' }}>
                      <Megaphone size={16} color="#6366f1" />
                      <div>
                        <div style={{ fontWeight: 600, color: '#f8fafc' }}>{c.name}</div>
                        <div style={{ fontSize: '0.8rem', color: '#64748b' }}>{c.template?.subject}</div>
                      </div>
                    </td>
                    <td style={{ padding: '1rem 0.5rem' }}>{c.audienceList?.name}</td>
                    <td style={{ padding: '1rem 0.5rem' }}>
                      <span className={`status-pill ${c.status === 'COMPLETED' ? 'status-sent' : 'status-pending'}`}>{c.status}</span>
                    </td>
                    <td style={{ padding: '1rem 0.5rem', color: '#64748b' }}>{new Date(c.createdAt).toLocaleString()}</td>
                  </tr>
                ))}
              </tbody>
            </table>
