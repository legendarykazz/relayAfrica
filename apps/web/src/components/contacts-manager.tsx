...
export default function ContactsManager({ apiKey }: { apiKey: string }) {
  ...
  return (
    <div className="animate-fade" style={{ display: 'flex', gap: '1rem', alignItems: 'flex-start' }}>
      ...
      <div className="panel glass" style={{ width: '240px', flexShrink: 0 }}>
        ...
        {selectedListId ? (
          <>
            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '1rem' }}>
              <div>
                <h2 style={{ fontSize: '1rem', fontWeight: 600 }}>Contacts Directory</h2>
                <p style={{ color: '#64748b', fontSize: '0.75rem' }}>Manage subscribers for {lists.find(l => l.id === selectedListId)?.name}</p>
              </div>
            ...
            {showNewContact && (
               <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '0.75rem', background: 'rgba(255,255,255,0.02)', padding: '1rem', borderRadius: '0.5rem', marginBottom: '1.5rem', border: '1px dashed rgba(255,255,255,0.1)' }}>
                 <div><label style={{display: 'block', fontSize: '0.7rem', color: '#94a3b8', marginBottom: '0.2rem'}}>Email *</label><input type="email" className="input-styled" value={newContact.email} onChange={e => setNewContact({...newContact, email: e.target.value})} style={{padding: '0.5rem'}} /></div>
                 <div><label style={{display: 'block', fontSize: '0.7rem', color: '#94a3b8', marginBottom: '0.2rem'}}>Phone</label><input type="tel" className="input-styled" placeholder="+123..." value={newContact.phone} onChange={e => setNewContact({...newContact, phone: e.target.value})} style={{padding: '0.5rem'}} /></div>
                 <div><label style={{display: 'block', fontSize: '0.7rem', color: '#94a3b8', marginBottom: '0.2rem'}}>First Name</label><input type="text" className="input-styled" value={newContact.firstName} onChange={e => setNewContact({...newContact, firstName: e.target.value})} style={{padding: '0.5rem'}} /></div>
                 <div><label style={{display: 'block', fontSize: '0.7rem', color: '#94a3b8', marginBottom: '0.2rem'}}>Last Name</label><input type="text" className="input-styled" value={newContact.lastName} onChange={e => setNewContact({...newContact, lastName: e.target.value})} style={{padding: '0.5rem'}} /></div>
                 <div style={{ gridColumn: 'span 2', display: 'flex', justifyContent: 'flex-end', gap: '0.75rem', marginTop: '0.4rem' }}>
                   <button className="btn" style={{ background: 'transparent', color: '#94a3b8', fontSize: '0.8rem' }} onClick={() => setShowNewContact(false)}>Cancel</button>
                   <button className="btn btn-primary" onClick={handleAddContact} style={{ fontSize: '0.8rem' }}>Save</button>
                 </div>
               </div>
            )}
                  ...
                  <tbody>
                    {contacts.map(c => (
                      <tr key={c.id} style={{ borderBottom: '1px solid rgba(255,255,255,0.05)' }}>
                        <td style={{ padding: '0.6rem 0.5rem', display: 'flex', alignItems: 'center', gap: '0.5rem' }}><Mail size={12} color="#64748b" /> {c.email}</td>
                        <td style={{ padding: '0.6rem 0.5rem' }}>{c.firstName || '-'} {c.lastName || '-'}</td>
                        <td style={{ padding: '0.6rem 0.5rem' }}>{c.phone || '-'}</td>
                        <td style={{ padding: '0.6rem 0.5rem', color: '#64748b' }}>{new Date(c.createdAt).toLocaleDateString()}</td>
                      </tr>
                    ))}
                  </tbody>
                </table>
